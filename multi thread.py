...
ALPHA = (34, 177, 76)
steps = 10
pygame.display.set_caption("multi thread")
'''
Objects
'''
...
class myThread (threading.Thread):
    ...
    def run(self):
        if self.threadID == 1:
            ...
            deb1()
        elif self.threadID == 11:
            deb2()
        elif self.threadID == 12:
            deb3()
        elif self.threadID == 13:
            dpb()
...
def collisioncheck():
    global player_list, main, k, colid, e, ed, edead, level, adead, playerdead, p1shot, enemycount, start, eshot,ge,en1,en2,en3,didtime
    global bullet_list, enemies1, enemies2, enemies3, enemiesbullets1, enemiesbullets2, enemiesbullets3, gen, playeralive
    global movex, movey, did, nets, mtime, tempx, tempy
    if e == True:
        for id, player in enumerate(player_list.sprites()):
            ...
            if playeralive[id] == 1:
                ...
                if bullet_list.sprites()[id].rect.colliderect(enemies1.sprites()[id].rect) and en1[id] == 1:
                    ...
                    enemiesbullets1.sprites()[id].kill()
                    '''
                    ge[id].fitness += 900000
                    edead[id] += 1

                    print("en1 dead", id, edead[id], "gen", gen)
                ...
                if player.rect.colliderect(boundry.rect) or player.rect.colliderect(
                    ...
                    behindb.rect):
                    player.die()
                    #print("boundry", id, edead[id], "gen", gen)
                    playeralive[id] = 0
                    #print(id)
                    ge[id].fitness -= 2000
...
def eval_genomes(genomes, config):
    global player_list, main, k, colid, e, ed, edead, level, adead, playerdead, p1shot, enemycount, start, eshot, ge, en1, en2, en3, didtime
    global bullet_list, enemies1, enemies2, enemies3, enemiesbullets1, enemiesbullets2, enemiesbullets3, gen, playeralive
    global movex, movey, did, nets, mtime, tempx, tempy, tcolli, tlevel, tebullet, tevent, tneat, threads
    population = 20
    e = False
    gen += 1
    enemycount = []
    ...
    while alive > 0:
        ...
        tneat = myThread(5)
        threads = []
        threads.append(tcolli)
        threads.append(tlevel)
        threads.append(tebullet)
        #threads.append(tevent)
        threads.append(tneat)
        tcolli.start()
        tlevel.start()
        tebullet.start()
        #tevent.start()
        ...
        tpb = myThread(13)
        threads2 = []
        threads2.append(tplayer)
        threads2.append(ten1)
        threads2.append(ten2)
        threads2.append(ten3)
        threads2.append(teb1)
        threads2.append(teb2)
        threads2.append(teb3)
        threads2.append(tpb)
